...
import { HANDLERS_FILE } from './HandlerTemplateGenerator.js';
import { STUB_FILE } from './MappingGenerator.js';
...
export function scaffold(options: ScaffoldOptions): ScaffoldResult {
  ...
  return {
    ...
    files: [
      ...
      drop(cwd, join(dir, INIT_FILE), initTemplate(dir)),
    ],
  };
}
...
function initTemplate(dir: string): string {
  const toRoot = relative(dir, '.').replace(/\\/g, '/') || '.';
  const map = `${toRoot}/${STUB_FILE.replace(/\.ts$/, '')}`;
  const handlers = `${toRoot}/${HANDLERS_FILE.replace(/\.ts$/, '')}`;
...
import * as requests from '${handlers}';
